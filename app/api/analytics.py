...
async def list_patients_with_unresolved_conflicts(
    ...
    try:
        ...
        rows = await database[PATIENTS_COLLECTION].aggregate(pipeline).to_list(length=limit)
        ...
        raise HTTPException(
            ...
            detail="Database operation failed while aggregating clinic unresolved conflicts.",
        ) from error
...
async def count_high_burden_clinics(
    ...
    try:
        ...
        rows = await database[CONFLICTS_COLLECTION].aggregate(pipeline).to_list(length=1000)
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Database operation failed while aggregating high-burden clinics.",
        ) from error
